---
title: "XGBoost Zillow Competition Notebook"
...
```{python load data}
props = pd.read_csv('data/properties_2016.csv', index_col = 'parcelid')
errors = pd.read_csv('data/train_2016_v2.csv', index_col = 'parcelid')
```

```{python merge}
df = errors.join(props, how = 'inner')
X = df.loc[:, ['bathroomcnt', 'bedroomcnt', 'calculatedbathnbr', 
                       'calculatedfinishedsquarefeet', 'fullbathcnt', 'lotsizesquarefeet',
...
```{python explore}
train.isnull().sum()
train.groupby('airconditioningtypeid').size()
```
```{python train-early-stopping}
params = {}
params['eta'] = 0.02
params['gamma'] = 0
...
evals = [(d_train, 'train'), (d_valid, 'validate')]

clf = xgb.train(params, d_train, evals = evals, num_boost_round = 10000, 
...
clf = xgb.cv(params, d_train, num_boost_round = 1000, verbose_eval = 10)
```
...
model_xgb = xgb.train(xgb_params, dtrain, 1000, evals = watchlist, early_stopping_rounds=100,
                  maximize=False, verbose_eval=10)

sns.histplot(data = df_train, x = "logerror")
plt.show()
...
# test on values with known outcomes
test_preds = model_xgb.predict(dtestknown)
r2_score(ytest, np.repeat(ytrain.mean(), len(ytest)))
mean_absolute_error(ytest, np.repeat(ytrain.mean(), len(ytest)))
...
```{python just-the-mean}

y = train.loc[:, 'logerror']
ymean = y.mean()

for c in test.columns[test.columns != 'parcelid']:
    test[c] = ymean
    
test.to_csv('data/submission.csv', index = False, float_format='%.4f')
```

```{python monthly-mean}
train = pd.read_csv('data/train_2016_v2.csv' , parse_dates=["transactiondate"], 
                    index_col = 'transactiondate')
month_means = []
for month in range(10, 13):
  month_means.append(train.loc[f'2016-{month}', 'logerror'].mean())
  

for c, m in zip([[1, 4], [2, 5], [3, 6]], month_means):
  test.iloc[:, c] = m
test.head()

test.to_csv('data/submission_monthly_mean.csv', index = False, float_format='%.4f')
```
...
watchlist = [(Xtrain, ytrain), (Xvalid, yvalid)]


reg = xgb.XGBRegressor(kwargs = xgb_params, early_stopping_rounds = 10)
reg.fit(X = Xtrain, y = ytrain, eval_set = watchlist)
